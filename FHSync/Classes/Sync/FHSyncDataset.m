        ...
        @try {
            [self doCloudCall:syncLoopParams
                   ...
                   AndFailure:^(FHResponse *response) {
                       // The AJAX call failed to complete successfully, so the state of the current
                       // pending updates is unknown
                       // Mark them as "crashed". The next time a syncLoop completes successfully, we
...
- (void)doCloudCall:(NSMutableDictionary *)params
    ...
    NSString *path = [NSString stringWithFormat:@"/sync/%@", self.datasetId];
...
- (void) checkUidChanges:(NSDictionary*) appliedUpdates
{
    if (appliedUpdates && [appliedUpdates count] > 0) {
        ...
        [appliedUpdates enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL * stop) {
            ...
            if ([action isEqualToString:@"create"]) {
                //we are receiving the results of creations, at this point, we will have the old uid(the hash) and the real uid generated by the cloud
